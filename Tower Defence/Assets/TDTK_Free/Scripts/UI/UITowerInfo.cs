...
namespace TDTK {
	...
	public class UITowerInfo : MonoBehaviour {
		...
		public void OnTargetPrioritySwitch(){
            ...
            if (currentTower.targetPriority == Unit._TargetPriority.First)
            {
                txtTargetPriority.text = "첫번째 적";
            }
            if (currentTower.targetPriority == Unit._TargetPriority.Nearest)
            {
                txtTargetPriority.text = "가까운 적";
            }
            
            if (currentTower.targetPriority == Unit._TargetPriority.Random)
            {
                txtTargetPriority.text = "랜덤";
            }
            if (currentTower.targetPriority == Unit._TargetPriority.Toughest)
            {
                txtTargetPriority.text = "순서대로";
            }
            if (currentTower.targetPriority == Unit._TargetPriority.Weakest)
            {
                txtTargetPriority.text = "체력이 적은 적";
            }
		...
		public void _SetScreenPos(Vector3 pos){
			if(pos.x<Screen.width/2){
				...
				frameT.localPosition=new Vector3(-370, 0, 0);
			}
			else{
